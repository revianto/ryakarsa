...
**0. Cek pola interaksi existing dulu (produk yang sudah jalan).** Sebelum membangun UI baru di project yang sudah punya kode jalan (bukan project kosong), scan dulu bagaimana pola interaksi yang SUDAH established di produk ini — bukan cuma token warna/font (itu tahap terpisah di `design-brief` tahap 1a), tapi konvensi *behavior*: cara loading state ditampilkan (skeleton vs spinner), notifikasi (toast vs inline banner vs modal), overlay pilihan (modal vs drawer vs bottom sheet), pola form validation yang sudah dipakai, penamaan microcopy tombol yang konsisten, dst. Cari dari komponen/halaman lain yang sudah ada di codebase (grep pola nama komponen umum: `Toast`, `Skeleton`, `Modal`, `Drawer`, `Spinner`, dll) atau dari `UI-STANDARDS.md` kalau sudah pernah dibuat lewat Standards mode.

- **Kalau pola existing ditemukan** → ikuti pola itu untuk kasus yang sama (jangan perkenalkan pola baru yang tidak konsisten, mis. pakai skeleton di halaman baru padahal seluruh produk selalu pakai spinner) — sebutkan eksplisit pola mana yang diikuti dan dari mana ditemukan. Poin 1-14 di bawah tetap berlaku sebagai baseline kualitas (kontras, tap target, dst.), tapi *pilihan pola*-nya ikut yang sudah ada, bukan pilihan bebas dari daftar di bawah.
- **Kalau ada gap** — situasi baru yang belum ada presedennya di produk ini (mis. produk belum pernah punya multi-step form, sekarang perlu) → baru pakai rule default di poin 1-14 di bawah, dan tandai eksplisit ke user bahwa ini pola baru (bukan mengikuti existing) supaya sadar ada keputusan konsistensi yang diambil.
- **Project kosong/baru** (belum ada kode UI sama sekali) → skip langkah ini, langsung ke poin 1.

1. **Tokens first.** Before writing any component, define design tokens: color roles (primary/secondary/accent/semantic), spacing scale, radius, shadow, typography scale, dan z-index/elevation scale (urutan naik: tooltip < dropdown < modal < toast). One source of truth lintas platform (Flutter + Next.js) — jangan hardcode nilai berulang. **Urutan mencari nilai token, jangan langsung mengarang**: (a) token yang sudah ada di codebase project ini (`tailwind.config.*`, CSS variables di `:root`, `theme.json`, dst.) → pakai itu; (b) kalau tidak ada, cek apakah skill `design-tokens` terpasang dan punya design system tersimpan yang relevan (`manage_library.py list`) — kalau user menyebut nama design system (mis. "pakai design system 'rebrew'") atau project ini jelas milik salah satu yang tersimpan, pakai token dari situ dan sebutkan yang mana; (c) baru kalau dua-duanya kosong, nyatakan asumsi token secara eksplisit (mis. "primary diasumsikan #2563EB karena tidak ada brand guideline") — jangan mengarang diam-diam, dan tawarkan singkat untuk menyimpannya ke `design-tokens` supaya reusable di project berikutnya.
...
9. **Build forms right.** Label di atas field — placeholder bukan label; validasi on-blur, bukan per-keystroke; pesan error dekat field terkait dan menyertakan cara memperbaiki; `input type`/keyboard sesuai (`email`, `tel`, `number`); atribut `autocomplete` yang tepat; toggle show/hide password. **Pilih komponen input berdasar jumlah opsi & kontrak interaksi, bukan default kebiasaan**: 2-5 opsi tunggal visible semua → radio; 6+ opsi atau ruang terbatas → dropdown; 20+ opsi → searchable/combobox; banyak pilihan disubmit bareng form → checkbox; satu aksi on/off efek instan (bukan disubmit) → toggle, bukan checkbox; 2-4 opsi yang sering di-switch & harus selalu terlihat sejajar → segmented control. Detail & rationale lengkap tiap pola: §6.1 `references/panduan-lengkap.md`.
...
2. **Exclude identitas visual by default.** Dokumen ini TIDAK berisi: palet warna spesifik (hex/token warna), pemilihan font family, atau layout/wireframe per halaman — itu ranah skill `design-brief`, bukan behavior standard ini. Rasio 60-30-10 dan skala tipografi (12/14/16/20/24/32px) boleh disebut sebagai *sistem/rule*, tapi tanpa nilai warna atau nama font aktual. Kalau user secara eksplisit minta palet/font/layout ikut dimasukkan, sertakan — jangan menahan permintaan eksplisit; atau arahkan ke `design-brief` kalau itu yang sebenarnya mereka mau.
...
- Minta identitas visual (warna/font/layout) dari PRD, bukan aturan interaksi: itu `design-brief`, bukan skill ini.
- Butuh nilai token konkret (warna/tipografi/spacing) dari design system yang sudah pernah disimpan user: `design-tokens` — lihat poin 1 Design mode untuk urutan mencarinya. Kalau di sesi ini kamu terpaksa mengarang token baru karena belum ada yang tersimpan, tawarkan menyimpannya ke sana supaya tidak diarang ulang di project berikutnya.
